...
parser.add_argument('--starting_index', type=int, default=0, help='the index from which you would like to to start')
parser.add_argument('--ending_index', type=int, default=100, help='the index from which you would like to to start')
args = parser.parse_args()
info_dict = {'high_level_nodes': {},'unsolved_instances_index':[],'low_level_nodes':{},'intial_cost':{},'final_cost':{},'time_taken':{}}
...
solution_dict = copy.deepcopy(info_dict)
...
# locations = []
# number_of_agents = 7
# while len(locations) < 2 * number_of_agents:
...
for the_problem in location_list[:int(args.ending_index)]:
    ...
    while open_list != []:
        ...
        if a == None:
            ...
            time.sleep(3)
            info_dict['time_taken'][args.starting_index + problem_counter] = float(final_time - initial_time)
            info_dict["high_level_nodes"][args.starting_index + problem_counter] = float(high_level_nodes)
            info_dict["intial_cost"][args.starting_index + problem_counter] = float(initial_cost)
            info_dict['final_cost'][args.starting_index + problem_counter] = float(present_node.path_cost)
            info_dict['low_level_nodes'][args.starting_index + problem_counter] = float(low_level_nodes)
            
            with open('results' + str(args.num_agents)+':'+str(args.starting_index) +'.txt', 'w') as f:
                json.dump(info_dict, f, indent=4) 
            time.sleep(5)
            break

        elif final_time - initial_time > 300: 
            info_dict["unsolved_instances_index"].append(args.starting_index + problem_counter - 1)
            info_dict['time_taken'][args.starting_index + problem_counter] = float(final_time - initial_time)
            info_dict["high_level_nodes"][args.starting_index + problem_counter] = float(high_level_nodes)
            info_dict["intial_cost"][args.starting_index + problem_counter] = float(initial_cost)
            info_dict['final_cost'][args.starting_index + problem_counter] = float(present_node.path_cost)
